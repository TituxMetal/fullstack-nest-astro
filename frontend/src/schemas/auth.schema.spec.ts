...
describe('loginSchema', () => {
  ...
  it('should reject empty identifier', () => {
    ...
    if (!result.success) {
      expect(result.error.errors[0].message).toBe('Identifier is required')
    }
  })
  ...
  it('should reject short identifier', () => {
    ...
    if (!result.success) {
      expect(result.error.errors[0].message).toBe('Identifier must be at least 3 characters long')
    }
  })
